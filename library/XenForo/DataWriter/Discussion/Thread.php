...
{
	protected $_supportTagging = true;
	...
	*/
	...
	protected function _handleForumMove($sourceForumId, $destinationForumId)
	{
		...
		if ($sourceForum && $destinationForum && $sourceForum['count_messages'] != $destinationForum['count_messages'])
		{
			...
			$oldCounted = ($oldState == 'visible' && $sourceForum['count_messages']);
			$newState = $this->get('discussion_state');
			$newCounted = ($newState == 'visible' && $destinationForum['count_messages']);
			...
			{
				// moving from a counting forum to a non-counting forum
