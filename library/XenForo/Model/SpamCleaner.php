...
class XenForo_Model_SpamCleaner extends XenForo_Model
{
	...
	protected function _banUser(array $user, array &$log, &$errorKey = '')
	{
		...
		$reason = new XenForo_Phrase('spam_cleaner_ban_reason');
		$reason = $reason->render();

		return $this->getModelFromCache('XenForo_Model_User')->ban(
			$user['user_id'], XenForo_Model_User::PERMANENT_BAN, $reason, $existing, $errorKey
		);
	}
	...
	public function prepareLogFetchOptions(array $fetchOptions)
	{
		...
		if (!empty($fetchOptions['join']))
		{
			...
			{
				$selectFields .= ',
					user.*, IF(user.username IS NULL, spam_cleaner_log.username, user.username) AS username';
				$joinTables .= '
