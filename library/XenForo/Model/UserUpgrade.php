...
class XenForo_Model_UserUpgrade extends XenForo_Model
{
	...
	public function prepareUserUpgrade(array $upgrade)
	{
		...
		$cost = XenForo_Locale::numberFormat($upgrade['cost_amount'], 2) . " $upgrade[currency]";
	...
	public function upgradeUser($userId, array $upgrade, $allowInsertUnpurchasable = false, $endDate = null)
	{
		...
		else
		{
			...
			$maxExpiredId = intval($db->fetchOne("
				SELECT MAX(user_upgrade_record_id)
				FROM xf_user_upgrade_expired
			"));
			if ($upgradeRecordId <= $maxExpiredId)
			{
				$result = $db->query("
					UPDATE IGNORE xf_user_upgrade_active
					SET user_upgrade_record_id = ?
					WHERE user_upgrade_record_id = ?
				", array($maxExpiredId + 1, $upgradeRecordId));
				if ($result->rowCount())
				{
					$upgradeRecordId = $maxExpiredId + 1;

					try
					{
						$newAi = $upgradeRecordId + 1;
						$db->query("
	...
	public function downgradeUserUpgrades(array $upgrades, $sendAlert = true)
	{
		...
		foreach ($upgrades AS $upgrade)
		{
			...
			if ($upgradeDef && $upgradeDef['recurring'] && $sendAlert)
			{
				// For recurring payments give a 24 hour grace period if it's a "natural" expiration (based on sendAlert)
				if ($upgrade['end_date'] + 86400 >= time())
				{
					continue;
				}
			}
