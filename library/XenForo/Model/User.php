...
class XenForo_Model_User extends XenForo_Model
{
	...
	public function updateSessionActivity($userId, $ip, $controllerName, $action, $viewState, array $inputParams, $viewDate = null, $robotKey = '')
	{
		...
		$controllerName = substr($controllerName, 0, 75);
	...
	public static $userContentChanges = array(
		...
		'xf_forum_watch' => array(array('user_id', false, false)),
		'xf_ip' => array(array('user_id', false, false)),
		...
		'xf_thread_reply_ban' => array(array('user_id', false, false), array('ban_user_id')),
		'xf_thread_user_post' => array(array('user_id', false, false)),
		'xf_thread_watch' => array(array('user_id', false, false)),
		'xf_upgrade_log' => array(array('user_id', false, false)),
	...
	public function mergeUsers(array $target, array $source)
	{
		...
		if ($dw->setExistingData($target))
		{
			...
			$this->updateFollowingDenormalizedValue($target['user_id']);
			$this->getModelFromCache('XenForo_Model_UserIgnore')->rebuildUserIgnoreCache($target['user_id']);
	...
	public function getUserExportXml(array $user)
	{
		$document = new DOMDocument('1.0', 'utf-8');
		$document->formatOutput = true;

		$rootNode = $document->createElement('data_portability_user');
		$document->appendChild($rootNode);

		$userNode = $this->_getUserNode($document, $user);
	...
	protected function _getUserNode(DOMDocument $document, array $user)
	{
		$attributes = array(
			'username',
			'email',
			...
			'dob_month',
			'dob_year',
			'homepage', // needs to be saved as website for XF2 compat
			'location'
		);
		$children = array(
			'signature',
		...
		foreach ($attributes AS $attribute)
		{
			...
			{
				$userNode->setAttribute('website', $user['homepage']);
			}
			else
			{
				$userNode->setAttribute($attribute, $user[$attribute]);
			}
		}
		foreach ($children AS $child)
		{
			if ($child == 'custom_fields' && $user['custom_fields'])
			{
				$fieldNode = $document->createElement('custom_fields');

				$user['custom_fields'] = @unserialize($user['custom_fields']);

				foreach ((array)$user['custom_fields'] AS $fieldId => $value)
				{
					$fieldChildNode = $document->createElement($fieldId);
					if (is_array($value))
					{
						$newNode = XenForo_Helper_DevelopmentXml::createDomCdataSection($document, json_encode($value));
						$fieldChildNode->setAttribute('array', 'true');
					}
					else
					{
						$newNode = XenForo_Helper_DevelopmentXml::createDomCdataSection($document, $value);
					}

					$fieldChildNode->appendChild($newNode);
					$fieldNode->appendChild($fieldChildNode);
				}

				$userNode->appendChild($fieldNode);
			}
			else
			{
				$fieldNode = $document->createElement($child);
				$newNode = XenForo_Helper_DevelopmentXml::createDomCdataSection($document, $user[$child]);
	...
	public function importUserXml(SimpleXMLElement $xml)
	{
		if ($xml->getName() != 'data_portability_user')
		{
			throw new XenForo_Exception(new XenForo_Phrase('please_provide_valid_xml_file'), true);
		}
		...
		if ($this->getUserByName($username) || $this->getUserByEmail($email))
		{
			throw new XenForo_Exception(new XenForo_Phrase('user_with_this_username_and_or_email_already_exists'), true);
		}
		...
		$userInput = array(
			'username' => $username,
			'email' => $email,
			'user_group_id' => self::$defaultRegisteredGroupId
		);
		...
		foreach ($userFields AS $field)
		{
			...
			{
				$userInput[$field] = (string)$xmlUser[$field];
			}
		}
		...
		foreach ($profileFields AS $field)
		{
			if (isset($xmlUser[$field]))
			{
				if ($field == 'website')
				{
					// for back/forwards compat with XF2 files
					$profileInput['homepage'] = (string)$xmlUser['website'];
				}
				else
				{
					$profileInput[$field] = (string)$xmlUser[$field];
				}
			}
		}
		...
		foreach ($profileElements AS $field)
		{
			$profileInput[$field] = (string)$xmlUser->$field;
		}

		$input = array_merge($userInput, $profileInput);
		$writer->bulkSet(XenForo_Input::cleanStringArray($input));

		$fieldsInfo = $this->_getDataRegistryModel()->get('userFieldsInfo');
		$xmlCustomFields = $xmlUser->custom_fields;

		$customFields = array();
		foreach (array_keys($fieldsInfo) AS $fieldId)
		{
			if ((string)$xmlCustomFields->$fieldId['array'])
			{
				$customFields[$fieldId] = json_decode((string)$xmlCustomFields->$fieldId, true);
			}
			else
			{
				$customFields[$fieldId] = (string)$xmlCustomFields->$fieldId;
			}
		}

		$writer->setCustomFields(XenForo_Input::cleanStringArray($customFields));

		// this is actually reset again, below, it's just to keep the DW happy
		// before the password can be reset again and emailed to the user.
		...
		if ($writer->hasErrors())
		{
			throw new XenForo_Exception($writer->getErrors(), true);
		}
