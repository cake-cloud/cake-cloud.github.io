...
class XenForo_ControllerAdmin_User extends XenForo_ControllerAdmin_Abstract
{
	...
	public function actionBatchUpdate()
	{
		if ($this->isConfirmedPost())
		{
			...
			if ($this->_input->filterSingle('confirmUpdate', XenForo_Input::UINT) && $actions)
			{
				...
				return $this->responseRedirect(
					XenForo_ControllerResponse_Redirect::SUCCESS,
					XenForo_Link::buildAdminLink('users/batch-update', null, array('success' => 1))
				);
			}
			else
			{
	...
	public function actionSave()
	{
		...
		$changePassword = $this->_input->filterSingle('change_password', XenForo_Input::BOOLEAN);
		...
		if ($this->_input->filterSingle('disable_tfa', XenForo_Input::BOOLEAN))
		{
			/** @var XenForo_Model_Tfa $tfaModel */
			$tfaModel = $this->getModelFromCache('XenForo_Model_Tfa');
			$tfaModel->disableTfaForUser($userId);
		}
	...
	public function actionAvatarUpload()
	{
		...
		return $this->responseRedirect(
			XenForo_ControllerResponse_Redirect::SUCCESS,
			XenForo_Link::buildAdminLink('users/edit', $user, array('success' => 1))
		);
	}
	...
	public function actionDelete()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			$rename = $this->_input->filterSingle('rename', XenForo_Input::BOOLEAN);
			if ($rename)
			{
				$renameTo = $this->_input->filterSingle('rename_to', XenForo_Input::STRING);
				if (!$renameTo)
				{
					return $this->responseError(new XenForo_Phrase('please_enter_name_to_rename_this_user_to'));
				}
				$writer->setOption(XenForo_DataWriter_User::OPTION_ADMIN_EDIT, true);
				$writer->set('username', $renameTo);
	...
	public function actionMerge()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			if ($mergeUser)
			{
				...
				return $this->responseRedirect(
					XenForo_ControllerResponse_Redirect::SUCCESS,
					XenForo_Link::buildAdminLink('users/edit', $mergeUser, array('success' => 1))
				);
			}
		}
	...
	public function actionResendConfirmation()
	{
		...
		if ($user['user_state'] != 'email_confirm' && $user['user_state'] != 'email_confirm_edit')
		{
			return $this->responseError(new XenForo_Phrase('this_users_account_does_not_require_confirmation'));
		}
		...
		{
			return $this->responseView('XenForo_ViewAdmin_User_ResendConfirmation', 'user_resend_confirmation', array(
				'user' => $user
			));
		}
	}
	...
	/**
	 * Displays the form to setup the alert, or to confirm sending of it.
	 *
	 * @return XenForo_ControllerResponse_Abstract
	 */
	public function actionAlert()
	{
		if ($this->isConfirmedPost())
		{
			...
			$alert = $this->_input->filter(array(
				'from_user' => XenForo_Input::STRING,

				'link_url' => XenForo_Input::STRING,
				'link_title' => XenForo_Input::STRING,
				'alert_body' => XenForo_Input::STRING
			));
			...
			if ($alert['from_user'])
			{
				$user = $userModel->getUserByName($alert['from_user']);
			}

			if ($alert['from_user'] && !$user)
			{
				return $this->responseError(new XenForo_Phrase('requested_user_x_not_found', array('name' => $alert['from_user'])));
			}
			...
			$total = $userModel->countUsers($criteriaPrepared);
			if (!$total)
			{
				return $this->responseError(new XenForo_Phrase('no_users_matched_specified_criteria'));
			}
			...
			return $this->responseView('XenForo_ViewAdmin_User_AlertConfirm', 'user_alert_confirm', $viewParams);
		}
		else
		{
			$viewParams = array(
			...
			return $this->responseView('XenForo_ViewAdmin_User_Alert', 'user_alert', $viewParams);
		}
	}

	/**
	 * Sends the specified alert.
	 *
	 * @return XenForo_ControllerResponse_Abstract
	 */
	public function actionAlertSend()
	{
		...
		{
			$data = array(
				'userIds' => array(XenForo_Visitor::getUserId()),
				'alert' => $alert
			);
			XenForo_Deferred_Abstract::create('UserAlert')->execute(array(), $data, 10, $null);

			return $this->responseReroute(__CLASS__, 'alert');
		}

		XenForo_Application::defer('UserAlert', array(
			'criteria' => $criteriaPrepared,
			'alert' => $alert
		), 'userAlertSend', true);

		return $this->responseRedirect(
			XenForo_ControllerResponse_Redirect::SUCCESS,
			XenForo_Link::buildAdminLink('users/alert', false, array(
				'sent' => $total
			))
		);
	}
	...
	public function actionEmailSend()
	{
		...
		XenForo_Application::defer('UserEmail', array(
			'criteria' => $criteriaPrepared,
			'email' => $email
		), 'userEmailSend', true);
	...
	/**
	 * Displays the form to setup the conversation, or to confirm sending of it.
	 *
	 * @return XenForo_ControllerResponse_Abstract
	 */
	public function actionMessage()
	{
		if ($this->isConfirmedPost())
		{
			...
			$conversation = $this->_input->filter(array(
				...
				'delete_type' => XenForo_Input::STRING
			));

			if (!$conversation['from_user'] || !$conversation['message_title'] || !$conversation['message_body'])
			{
				return $this->responseError(new XenForo_Phrase('please_complete_required_fields'));
			}
			...
			$userModel = $this->_getUserModel();
			$user = $userModel->getUserByName($conversation['from_user']);

			if (!$user)
			{
				return $this->responseError(new XenForo_Phrase('requested_user_x_not_found', array('name' => $conversation['from_user'])));
			}
			...
			return $this->responseView('XenForo_ViewAdmin_User_MessageConfirm', 'user_message_confirm', $viewParams);
		}
		else
		{
			$viewParams = array(
			...
			return $this->responseView('XenForo_ViewAdmin_User_Message', 'user_message', $viewParams);
		}
	}
	...
	public function actionMessageSend()
	{
		...
		XenForo_Application::defer('UserMessage', array(
			'criteria' => $criteriaPrepared,
			'conversation' => $conversation
		), 'userMessageSend', true);

		return $this->responseRedirect(
			XenForo_ControllerResponse_Redirect::SUCCESS,
			XenForo_Link::buildAdminLink('users/message', false, array(
				'sent' => $total
			))
		);
	}
