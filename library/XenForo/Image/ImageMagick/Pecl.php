...
class XenForo_Image_ImageMagick_Pecl extends XenForo_Image_Abstract
{
	...
	{
		if (method_exists($this->_image, 'getImageProfiles'))
		{
			$profiles = $this->_image->getImageProfiles('icc');
			$this->_image->stripImage();

			if ($profiles && !empty($profiles['icc']))
			{
				$this->_image->setImageProfile('icc', $profiles['icc']);
			}
		}
		else
		{
		...
		switch ($outputType)
		{
			case IMAGETYPE_GIF:
				...
				{
					$optimized = @$this->_image->optimizeimagelayers();
					if ($optimized instanceof \Imagick)
					{
						$this->_image = $optimized;
					}

					$deconstructed = @$this->_image->deconstructImages();
					if ($deconstructed instanceof \Imagick)
					{
						$this->_image = $deconstructed;
					}
				}
