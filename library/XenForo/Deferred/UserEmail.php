...
class XenForo_Deferred_UserEmail extends XenForo_Deferred_Abstract
{
	...
	protected function _sendEmail(array $email, array $user, Zend_Mail_Transport_Abstract $transport)
	{
		...
		$unsubPhrase = $phraseModel->getPhraseTextFromPhraseTitles('unsubscribe_from_mailing_list', $user['language_id']);
		$unsubPhrase = reset($unsubPhrase);

		$unsubLink = XenForo_Link::buildPublicLink('canonical:email-stop/mailing-list', null, array(
			'u' => $user['user_id'],
			'c' => XenForo_Model::create('XenForo_Model_User')->getUserEmailConfirmKey($user)
		));
		$unsubPlain = $unsubPhrase . ' - ' . $unsubLink;
		...
		if ($email['email_format'] == 'html')
		{
			...
			if (strpos($text, '{unsub}') !== false)
			{
				$text = str_replace('{unsub}', $unsubPlain, $text);
			}
			if (strpos($email['email_body'], '{unsub}') !== false)
			{
				$email['email_body'] = str_replace('{unsub}', '<div style="font-size: 11px; color: rgb(150,150,150);" align="center">'
					. '<a style="color: rgb(150,150,150); text-decoration: underline;" href="' . $unsubLink . '">' . $unsubPhrase . '</a>'
					. '</div>', $email['email_body']);
			}
