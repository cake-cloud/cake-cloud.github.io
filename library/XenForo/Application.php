...
class XenForo_Application extends Zend_Registry
{
	 ...
	 * @var integer
	 */
	public static $version = '1.5.24';
	public static $versionId = 1052470; // abbccde = a.b.c d (alpha: 1, beta: 3, RC: 5, stable: 7, PL: 9) e
	...
	*/
	...
	protected static $_randomSource;
	...
	public static function generateRandomString($length, $raw = false)
	{
		...
		while (strlen(self::$_randomData) < $length)
		{
			...
			{
				self::$_randomData .= random_bytes($length);
				self::$_randomSource = 'random_bytes';
			}
			else if (function_exists('mcrypt_create_iv') && PHP_VERSION_ID >= 50307)
			{
				self::$_randomData .= mcrypt_create_iv($length, MCRYPT_DEV_URANDOM);
				self::$_randomSource = 'mcrypt_create_iv';
			}
			...
			)
			{
				...
				self::$_randomSource = 'urandom';
			}
			else if (function_exists('openssl_random_pseudo_bytes')
				&& (substr(PHP_OS, 0, 3) != 'WIN' || PHP_VERSION_ID >= 50304)
			)
			{
				self::$_randomData .= openssl_random_pseudo_bytes($length);
				$mixInternal = true;

				self::$_randomSource = 'openssl_random_pseudo_bytes';
			}
			else
			{
				self::$_randomData .= self::generateInternalRandomValue();
				self::$_randomSource = 'internal';
			}
		}
		...
		// this adds fork safety if needed (https://wiki.openssl.org/index.php/Random_fork-safety)
		if ($mixInternal)
		{
			$final = '';
			foreach (str_split($return, 20) AS $i => $part)
			{
				$internal = uniqid(mt_rand());
				if ($i % 2 == 0)
				{
					$final .= sha1($part . $internal, true);
				}
				else
				{
					$final .= sha1($internal . $part, true);
				}
			}
	...
	public static function generateInternalRandomValue()
	{
		...
		{
			self::$_randomState = sha1(memory_get_usage() . getmypid()
				. serialize($_ENV) . serialize($_SERVER) . mt_rand() . microtime(), true);
		}

		$data = uniqid(mt_rand(), true) . memory_get_usage() . microtime() . self::$_randomState;
		self::$_randomState = sha1($data, true);

		return substr(self::$_randomState, 0, 10);
	}

	public static function getRandomValueSource()
	{
		if (!self::$_randomSource)
		{
			self::generateRandomString(1);
		}
