...
class XenForo_Dependencies_Public extends XenForo_Dependencies_Abstract
{
	...
	*/
	public function getEffectiveContainerParams(array $params, Zend_Controller_Request_Http $request)
	{
		...
		$params['showCookieNotice'] = (!XenForo_Visitor::getInstance()->user_id && $options->showFirstCookieNotice);
	...
	protected function _preloadNoticeTemplates(array $params, Zend_Controller_Request_Http $request)
	{
		...
		if (!empty($params['showCookieNotice']))
		{
			$this->preloadTemplate('notice_cookies');
		}
	}
	...
	protected function _getNavigationContainerParams($selectedTabId)
	{
		...
		{
			$homeTab = reset($extraTabsGrouped['home']);
			$homeTabId = key($extraTabsGrouped['home']);
		}
		else
		{
			$homeTabId = false;
	...
	/**
	 * Get the URL to the contact form
	 *
	 * @return string
	 */
	public static function getContactUrl()
	{
		...
		{
			case 'default': return XenForo_Link::buildPublicLink('misc/contact'); break;
			case 'custom': return $options->contactUrl['custom']; break;
			default: return '';
		}
	}

	/**
	 * Get the URL to the privacy policy
	 *
	 * @return string
	 */
	public static function getPrivacyPolicyUrl()
	{
		...
		{
			case 'default': return XenForo_Link::buildPublicLink('help/privacy-policy'); break;
			case 'custom': return $options->privacyPolicyUrl['custom']; break;
			default: return '';
		}
	}
	...
	public static function getJquerySource($forceLocal = false)
	{
		...
		switch ($source)
		{
			case 'jquery':
			case 'mt':
				return "https://code.jquery.com/jquery-{$jQueryVersion}{$min}.js";

			case 'google':
				return "https://ajax.googleapis.com/ajax/libs/jquery/{$jQueryVersion}/jquery{$min}.js";

			case 'microsoft':
				return "https://ajax.microsoft.com/ajax/jquery/jquery-{$jQueryVersion}{$min}.js";
