...
class XenForo_Importer_XenForo extends XenForo_Importer_Abstract
{
	...
	public static function processAttachmentTags($oldAttachmentId, $newAttachmentId, $messageText)
	{
		if (stripos($messageText, '[ATTACH') !== false)
		{
			$messageText = preg_replace(
				"#(\[ATTACH[^\]]*\]){$oldAttachmentId}(\[/ATTACH\])#siU",
				'${1}' . $newAttachmentId . '${2}',
				$messageText
			);
		}
	...
	public function stepLikes($start, array $options)
	{
		...
		$userIdMap = $model->getImportContentMap('user', $mapUserIds);

		$contentIdMap = array();
		foreach ($mapContentIds AS $contentType => $contentIds)
		{
			$contentIdMap[$contentType] = $model->getImportContentMap($contentType, array_unique($contentIds));
		}

		foreach ($likes AS $like)
		{
			$contentId = $this->_mapLookUp($contentIdMap[$like['content_type']], $like['content_id']);
			if (!$contentId)
			{
				continue;
			}

			$likeUserId = $this->_mapLookUp($userIdMap, $like['like_user_id']);
			if (!$likeUserId)
			{
				continue;
			}

			$contentUserId = $this->_mapLookUp($userIdMap, $like['content_user_id'], 0);
