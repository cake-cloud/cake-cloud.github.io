...
class XenForo_Importer_vBulletin extends XenForo_Importer_Abstract
{
	...
	public function getSteps()
	{
		...
		// deferred: announcements, custom bb code, calendars/events, social groups
	}
	...
	protected function _importUser(array $user, array $options)
	{
		...
		if ($user['customtitle'])
		{
			...
			if ($user['customtitle'] == 1) // admin set
			{
	...
	public function stepContentTags($start, array $options)
	{
		...
		$threadIdMap = $model->getThreadIdsMapFromArray($tags, 'threadid');
		$userIdMap = $model->getThreadIdsMapFromArray($tags, 'userid');

		foreach ($tags AS $tag)
		{
			$newThreadId = $this->_mapLookUp($threadIdMap, $tag['threadid']);
			if (!$newThreadId)
			{
				continue;
			}
	...
	public function stepAttachments($start, array $options)
	{
		...
		foreach ($attachments AS $attachment)
		{
			...
			else
			{
				...
				{
					$attachFileOrig = "$options[path]/" . $attachment['userid'] . "/$attachment[attachmentid].attach";
					if (!file_exists($attachFileOrig))
					{
						continue;
					}
