...
abstract class XenForo_ControllerPublic_Abstract extends XenForo_Controller
{
	...
	final protected function _preDispatchType($action)
	{
		...
		$this->_assertPolicyAcceptance($action);
	...
	{
		$visitor = XenForo_Visitor::getInstance();
		$privacyLastUpdate = XenForo_Application::getOptions()->privacyPolicyLastUpdate;
		$termsLastUpdate = XenForo_Application::getOptions()->termsLastUpdate;
		...
		)
		{
			throw $this->responseException(
				$this->responseRedirect(
					XenForo_ControllerResponse_Redirect::SUCCESS,
					XenForo_Link::buildPublicLink('misc/accept-privacy-policy', null, array(
						'redirect' => $this->_request->getRequestUri()
					))
				)
			);
		...
		)
		{
			throw $this->responseException(
				$this->responseRedirect(
					XenForo_ControllerResponse_Redirect::SUCCESS,
					XenForo_Link::buildPublicLink('misc/accept-terms', null, array(
						'redirect' => $this->_request->getRequestUri()
					))
				)
			);
