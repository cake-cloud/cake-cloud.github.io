...
class XenForo_ControllerPublic_Register extends XenForo_ControllerPublic_Abstract
{
	...
	protected function _getRegistrationInputData()
	{
		...
		if (XenForo_Dependencies_Public::getPrivacyPolicyUrl())
		{
			$data['privacy_policy_accepted'] = XenForo_Application::$time;
		}
		if (XenForo_Dependencies_Public::getTosUrl())
		{
			$data['terms_accepted'] = XenForo_Application::$time;
		}
	...
	public function actionRegister()
	{
		...
		if ((XenForo_Dependencies_Public::getTosUrl() || XenForo_Dependencies_Public::getPrivacyPolicyUrl()) && !$this->_input->filterSingle('agree', XenForo_Input::BOOLEAN))
		{
		...
		if ($this->_input->inRequest('email_choice'))
		{
			$choice = $this->_input->filterSingle('email_choice', XenForo_Input::BOOLEAN);
			$data['receive_admin_email'] = $choice;
		}
		$writer->bulkSet($data);
		...
		{
			$fields = $data;
			$fields['tos'] = $this->_input->filterSingle('agree', XenForo_Input::BOOLEAN);
	...
	protected function _completeRegistration(array $user, array $extraParams = array())
	{
		...
		/** @var XenForo_Model_UserChangeLog $userChangeLogModel */
		$userChangeLogModel = $this->getModelFromCache('XenForo_Model_UserChangeLog');

		$changes = array();

		if (XenForo_Application::getOptions()->get('registrationSetup', 'requireEmailChoice'))
		{
			$changes['receive_admin_email'] = array(0, $user['receive_admin_email'] ? 1 : 0);
		}
		if (XenForo_Dependencies_Public::getPrivacyPolicyUrl())
		{
			$changes['privacy_policy_accepted'] = array(0, XenForo_Application::$time);
		}
		if (XenForo_Dependencies_Public::getTosUrl())
		{
			$changes['terms_accepted'] = array(0, XenForo_Application::$time);
		}

		$userChangeLogModel->logChanges($user['user_id'], $changes, $user['user_id']);
	...
	public function actionFacebook()
	{
		...
		$fbRedirectUri = XenForo_Link::buildPublicLink('canonical:register/facebook');
	...
	public function actionTwitter()
	{
		$oauth = XenForo_Helper_Twitter::getOauthConsumer(
			XenForo_Link::buildPublicLink('canonical:register/twitter')
		);
		if (!$oauth)
		{
			return $this->responseRedirect(
	...
	public function actionGoogle()
	{
		...
		$client = XenForo_Helper_Http::getClient('https://oauth2.googleapis.com/token');
	...
	protected function _setupExternalUser(array $data)
	{
		...
		if ((XenForo_Dependencies_Public::getTosUrl() || XenForo_Dependencies_Public::getPrivacyPolicyUrl()) && !$this->_input->filterSingle('agree', XenForo_Input::UINT))
		{
			throw $this->responseException(
