...
class XenForo_Helper_UserChangeLog
{
	protected $_fieldInfo = array(
		...
		'use_tfa'                     => array('two_step_verification_enabled', '_prepareYesNoField'),
		'privacy_policy_accepted'     => array('accepted_privacy_policy', '_prepareDateField'),
		'terms_accepted'              => array('accepted_terms_rules', '_prepareDateField'),
	...
	public function isFieldProtected($fieldId)
	{
		return in_array($fieldId, $this->_protectedFields);
	}
	...
	protected function _prepareCustomField($fieldName, array $field)
	{
		...
		if (isset($userFieldInfo[$fieldName]))
		{
			...
			else if ($fieldInfo['field_type'] == 'radio' || $fieldInfo['field_type'] == 'select')
			{
				...
				else if (isset($fieldChoices[$field['old_value']]))
				{
				...
				else if (isset($fieldChoices[$field['new_value']]))
				{
