 ...
 */
!function(d,f,g,h){XenForo.SpamCleaner=function(b){var c=b.closest(".xenOverlay");c.length&&b.submit(function(e){e.preventDefault();XenForo.ajax(b.attr("action"),b.serializeArray(),function(a,e){if(XenForo.hasResponseError(a))return!1;XenForo.hasTemplateHtml(a)?new XenForo.ExtLoader(a,function(){b.slideUp(XenForo.speed.fast,function(){b.remove();$template=d(a.templateHtml).prepend('<h2 class="heading">'+a.title+"</h2>");$template.xfInsert("appendTo",c,"slideDown",XenForo.speed.fast);c.data("overlay").getTrigger().bind("onClose",
