...
!function($, window, document, _undefined)
{
	...
	XenForo.SelectQuotable.prototype =
	{
		...
		_isValidSelection: function(selection)
		{
			...
			if ($(range.startContainer).closest('.bbCodeQuote, .NoSelectToQuote').length
		        || $(range.endContainer).closest('.bbCodeQuote, .NoSelectToQuote').length)
	        {
		        return false;
	        }
		...
		getButtonPositionMarker: function(selection)
		{
			...
			if (!$.browser.msie) // IE loses the selection from this
			{
	...
	XenForo.unparseBbCode = function(html)
	{
		...
		$div.find('.NoSelectToQuote').each(function()
		{
			$(this).remove();
		});
